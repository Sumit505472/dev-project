...
const ProblemDetail = () => {
    ...
    const fetchProblem = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/problem/${id}`
        );
  ...
  return (
    <div
      ref={containerRef}
      className="flex h-[90vh] bg-gray-50 rounded shadow overflow-hidden select-none"
    >
      {/* Left panel */}
      {!fullscreen && (
        <div
        ...
        >
          ...
          <button>{problem.difficulty}</button>
          <p className="mb-4 text-2xl whitespace-pre-wrap">
            {problem.question_description || problem.description}
          </p>
          <h3 className="text-1xl font-bold mb-3">Input Format</h3>
          <p className="mb-4 text-gray-700 whitespace">
            {problem.input_format}
          </p>
          <h3 className="text-1xl font-bold mb-3">Output Format</h3>
          <p className="mb-4 text-gray-700 whitespace-pre-wrap">
            {problem.output_format}
          </p>
          <h3 className="text-1xl font-bold mb-3">Sample Input</h3>
          <p className="mb-4 text-gray-700 whitespace-pre-wrap border-black border-2">
            {problem.test_cases[0].input}
          </p>
          <h3 className="text-1xl font-bold mb-3">Sample Output</h3>
          <p className="mb-4 text-gray-700 whitespace-pre-wrap border-black border-2">
            {problem.test_cases[0].output}
          </p>
          <h3 className="text-2xl font-bold mb-3">Constraints :</h3>
          <p className="mb-4 text-gray-700 ">{problem.constraints}</p>
          <h3 className="text-2xl font-bold mb-3">Tags</h3>
          <p className="mb-4 text-gray-700 ">{problem.tags.join(", ")}</p>
          <h1 className="text-2xl font-bold mb-3">Time Limit</h1>
          <p className="mb-4 text-gray-700 ">{problem.time_limit}</p>
          <h1 className="text-2xl font-bold mb-3">Memory Limit</h1>
          <p className="mb-4 text-gray-700 ">{problem.memory_limit}</p>
        </div>
      )}
          ...
          showInput && (
            <div className="border-t border-gray-300 bg-gray-50 p-4 space-y-4 max-h-60 overflow-auto">
              ...
              <textarea
                ...
                style={{
                  height: `${(100 - editorHeightPercent) / 2}vh`,
                  minHeight: "60px",
                }}
              />
